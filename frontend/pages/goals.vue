        ...
        <v-container class="py-8">
          ...
          <v-row v-else>
            <v-col v-for="goal in goals" :key="goal.id" cols="12" md="3">
              <v-card class="goal-card" :class="priorityClass(goal.priority)" elevation="6">
                ...
                <v-card-text>
                  ...
                  <p class="goal-expiry">
                    <v-icon color="red">mdi-calendar</v-icon> Scadenza: {{ formatDate(goal.exp) }}
                  </p>
                </v-card-text>
              </v-card>
            </v-col>
